...
public class HorsePerms extends CustomCommand implements TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args[0].equals("add")) {
            ...
            if (reciever == null) {
                sender.sendMessage(Format.colorize("&cPlayer not found."));
                return true;
            }
            else if (reciever.getName().equals(player.getName())) {
                sender.sendMessage(Format.colorize("&cYou cannot add yourself as a horse owner."));
                return true;
            }

            boolean success = database.addReciever(player.getUniqueId(), reciever.getUniqueId());
            if (success) {
                player.sendMessage(Format.colorize("&aYou have added &e" + reciever.getName() + "&a to your horse permissions."));
            }
            else {
                player.sendMessage(Format.colorize("&e" + reciever.getName() + " &cis already on your horse permissions."));
            }
        }
        else if (args[0].equals("remove")) {
            OfflinePlayer reciever = Bukkit.getOfflinePlayer(args[1]);
            if (reciever.getName().equals(player.getName())) {
                sender.sendMessage(Format.colorize("&cYou cannot remove yourself as a horse owner."));
                return true;
            }
            ...
            if (success) {
                player.sendMessage(Format.colorize("&aYou have removed &e" + reciever.getName() + "&a from your horse permissions."));
                ...
                if (reciever.isOnline()) {
                    ...
                    if (onlineReciever.isInsideVehicle() && onlineReciever.getVehicle() instanceof Horse) {
                        ...
                        onlineReciever.sendMessage(Format.colorize("&e" + player.getName() + " &chas just removed you from their horse permissions, and you were kicked off their horse."));
                        onlineReciever.playSound(onlineReciever.getLocation(), Sound.ENTITY_VILLAGER_NO, 1, 1);
                    } 
                }
            }
            else {
                player.sendMessage(Format.colorize("&e" + reciever.getName() + " &cis not on your horse permissions."));
            }
        }
            ...
            if (args.length == 1) {
                ...
                try {
                    ...
                    player.sendMessage(Format.colorize("&cInvalid page number."));
                    return true;
                }
            ...
            if (maxPages == 0) {
                player.sendMessage(Format.colorize("&cYou have no one on your horse permissions."));
                return true;
            }
            ...
            if (page == 1) {
                backArrow = new TextComponent(Format.colorize("&7&l<< "));
            }
            else {
                backArrow = new TextComponent(Format.colorize("&6&l<< "));
                backArrow.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(Format.colorize("&6Go to page " + (page - 1)))));
                backArrow.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/horseperms list " + (page - 1)));
            }

            TextComponent forwardArrow;
            if (page == maxPages) {
                forwardArrow = new TextComponent(Format.colorize("&7&l >>"));
            }
            else {
                forwardArrow = new TextComponent(Format.colorize("&6&l >>"));
                forwardArrow.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(Format.colorize("&6Go to page " + (page + 1)))));
                forwardArrow.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/horseperms list " + (page + 1)));
            }

            TextComponent centerText = new TextComponent(Format.colorize("&aYour horse permissions: &e(Page " + page + " of " + maxPages + ")"));
            ...
            for (OfflinePlayer reciever : recievers) {
                player.sendMessage(Format.colorize("&e- " + reciever.getName()));
            }
            player.sendMessage(BORDER_LINE);
        }
